...
@pytest.mark.parametrize(
    "test_input,expected",
    [
        (get_aval(jax.numpy.ones(1)), fenics.Constant(0.0)),
        (get_aval(jax.numpy.ones(2)), fenics.Constant([0.0, 0.0])),
        (jax.numpy.asarray(0.66), fenics.Constant(0.66)),
        (
            jax.numpy.asarray([0.5, 0.66]),
...
@pytest.mark.parametrize(
    ...
    [
        (get_aval(jax.numpy.ones(10)), "0.0"),
        (ad_util.Zero(get_aval(jax.numpy.asarray(0.0))), "0.0"),
        (jax.numpy.linspace(0.05, 0.95, num=10), "x[0]"),
    ],
)
