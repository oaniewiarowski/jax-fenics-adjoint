...
def _aval_to_numpy(aval) -> np.array:
    if hasattr(aval, "val"):
        return np.asarray(aval.val)
    warnings.warn(
...
def jax_to_fenics_numpy(jax_array: JAXArray, fem_variable: BackendVariable) -> np.array:
    ...
    # JAX tracer specific part. Here we return zero values if tracer is not concrete.
    if isinstance(jax_array, ad_util.Zero):
        ...
        return _aval_to_numpy(jax.core.get_aval(jax_array))

    elif isinstance(jax_array, (jax.core.ShapedArray,)):
        return _aval_to_numpy(jax_array)
